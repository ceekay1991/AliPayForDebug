//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "O2OMistModel.h"

@class NSDictionary, NSString, O2OListViewController;

@interface O2OAllMyCommentsListModel : O2OMistModel
{
    _Bool _isReloadResult;
    _Bool _isFirst;
    NSDictionary *_monitorDic;
    NSString *_totalCommentSum;
    O2OListViewController *_vc;
    NSString *_pushProtocolUrl;
    NSString *_cdpCode;
    NSString *_lastCommentId;
}

@property(nonatomic) _Bool isFirst; // @synthesize isFirst=_isFirst;
@property(retain, nonatomic) NSString *lastCommentId; // @synthesize lastCommentId=_lastCommentId;
@property(nonatomic) _Bool isReloadResult; // @synthesize isReloadResult=_isReloadResult;
@property(retain, nonatomic) NSString *cdpCode; // @synthesize cdpCode=_cdpCode;
@property(retain, nonatomic) NSString *pushProtocolUrl; // @synthesize pushProtocolUrl=_pushProtocolUrl;
@property(nonatomic) __weak O2OListViewController *vc; // @synthesize vc=_vc;
@property(retain, nonatomic) NSString *totalCommentSum; // @synthesize totalCommentSum=_totalCommentSum;
@property(retain, nonatomic) NSDictionary *monitorDic; // @synthesize monitorDic=_monitorDic;
- (void).cxx_destruct;
- (id)responseObjects:(id)arg1;
- (id)templateConfig;
- (id)operationType;
- (SEL)selectorNameForRPCService;
- (Class)RPCServiceClass;
- (Class)RPCResponseClass;
- (Class)RPCRequestClass;
- (struct requestConfig)requestConfig;
- (id)cacheKey;
- (id)rpcRequest;
- (long long)pageSize;
- (_Bool)loadMore;
- (void)load;
- (void)reload;
- (_Bool)raise1002Exception;

@end

