//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "O2OHTTPModel.h"

@class NSString;

@interface O2ONearbyDiscountModel : O2OHTTPModel
{
    NSString *_shopX;
    NSString *_shopY;
    NSString *_shopId;
}

@property(copy, nonatomic) NSString *shopId; // @synthesize shopId=_shopId;
@property(copy, nonatomic) NSString *shopY; // @synthesize shopY=_shopY;
@property(copy, nonatomic) NSString *shopX; // @synthesize shopX=_shopX;
- (void).cxx_destruct;
- (_Bool)parseResponse:(id)arg1;
- (id)operationType;
- (SEL)selectorNameForRPCService;
- (Class)RPCServiceClass;
- (Class)RPCResponseClass;
- (Class)RPCRequestClass;
- (id)rpcRequest;

@end

