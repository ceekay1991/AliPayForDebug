//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "APDPBGeneratedMessage.h"

@class NSString;

@interface finscbffSignalItemPB : APDPBGeneratedMessage
{
}

+ (CDStruct_af61540b *)_fieldInfos;

// Remaining properties
@property(retain, nonatomic) NSString *date; // @dynamic date;
@property(readonly) _Bool hasDate; // @dynamic hasDate;
@property(readonly) _Bool hasIndex; // @dynamic hasIndex;
@property(readonly) _Bool hasSignalText; // @dynamic hasSignalText;
@property(readonly) _Bool hasType; // @dynamic hasType;
@property(nonatomic) int index; // @dynamic index;
@property(retain, nonatomic) NSString *signalText; // @dynamic signalText;
@property(nonatomic) int type; // @dynamic type;

@end

