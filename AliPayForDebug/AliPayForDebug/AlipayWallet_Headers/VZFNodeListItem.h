//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "VZListItem.h"

#import "VZFNodeProvider-Protocol.h"

@class NSString, UIView, VZFNodeListItemRecycler, VZFluxStore;

@interface VZFNodeListItem : VZListItem <VZFNodeProvider>
{
    _Bool _skipSameNode;
    _Bool _useGestureManager;
    VZFluxStore *_store;
    UIView *_attachedView;
    VZFNodeListItemRecycler *_recycler;
}

+ (id)nodeForItem:(id)arg1 Store:(id)arg2 Context:(id)arg3;
@property(retain, nonatomic) VZFNodeListItemRecycler *recycler; // @synthesize recycler=_recycler;
@property(nonatomic) _Bool useGestureManager; // @synthesize useGestureManager=_useGestureManager;
@property(nonatomic) _Bool skipSameNode; // @synthesize skipSameNode=_skipSameNode;
@property(readonly, nonatomic) __weak UIView *attachedView; // @synthesize attachedView=_attachedView;
@property(retain, nonatomic) VZFluxStore *store; // @synthesize store=_store;
- (void).cxx_destruct;
- (void)replaceLayout:(void *)arg1 node:(id)arg2;
- (void)detachFromView;
- (void)attachToView:(id)arg1;
- (void)updateState;
- (void)updateModel:(id)arg1 constrainedSize:(struct CGSize)arg2 context:(id)arg3;
- (void)dealloc;
- (id)init;
@property(readonly, nonatomic) float itemWidth;
@property(readonly, nonatomic) float itemHeight;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

