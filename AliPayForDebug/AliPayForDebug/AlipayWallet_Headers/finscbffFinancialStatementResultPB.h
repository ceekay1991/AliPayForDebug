//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "APDPBGeneratedMessage.h"

@class NSArray, NSString;

@interface finscbffFinancialStatementResultPB : APDPBGeneratedMessage
{
}

+ (CDStruct_af61540b *)_fieldInfos;

// Remaining properties
@property(retain, nonatomic) NSString *actionUrl; // @dynamic actionUrl;
@property(retain, nonatomic) NSString *bigNewsAlertDesc; // @dynamic bigNewsAlertDesc;
@property(retain, nonatomic) NSArray *financialStatementBlocks; // @dynamic financialStatementBlocks;
@property(readonly) _Bool hasActionUrl; // @dynamic hasActionUrl;
@property(readonly) _Bool hasBigNewsAlertDesc; // @dynamic hasBigNewsAlertDesc;
@property(readonly) _Bool hasIsHasBigNews; // @dynamic hasIsHasBigNews;
@property(readonly) _Bool hasReportDateTimestamp; // @dynamic hasReportDateTimestamp;
@property(readonly) _Bool hasResultCode; // @dynamic hasResultCode;
@property(readonly) _Bool hasResultDesc; // @dynamic hasResultDesc;
@property(readonly) _Bool hasResultView; // @dynamic hasResultView;
@property(readonly) _Bool hasShowRedPoint; // @dynamic hasShowRedPoint;
@property(readonly) _Bool hasSuccess; // @dynamic hasSuccess;
@property(nonatomic) _Bool isHasBigNews; // @dynamic isHasBigNews;
@property(nonatomic) long long reportDateTimestamp; // @dynamic reportDateTimestamp;
@property(retain, nonatomic) NSString *resultCode; // @dynamic resultCode;
@property(retain, nonatomic) NSString *resultDesc; // @dynamic resultDesc;
@property(retain, nonatomic) NSString *resultView; // @dynamic resultView;
@property(nonatomic) _Bool showRedPoint; // @dynamic showRedPoint;
@property(nonatomic) _Bool success; // @dynamic success;

@end

