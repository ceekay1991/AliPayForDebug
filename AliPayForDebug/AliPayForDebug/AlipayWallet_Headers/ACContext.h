...
@class ACAppDataManager, ACCleanAppTask, ACDownloadManager, ACLanguage, ACPreloadTask, ACRecentUseManager, ACRequestManager, ACStageDataManager, ACSyncCenter, ACZipArchiveManager, OPConfigCenter;
@protocol OPAdapterProtocol;
...
@interface ACContext : NSObject
{
    ...
    id <OPAdapterProtocol> _adapterPlugin;
    OPConfigCenter *_configCenter;
}

+ (id)configCenter;
+ (id)adapterPlugin;
+ (id)shareDevelopingAppManager;
+ (id)shareCommonRequestManager;
...
+ (id)shareContext;
+ (_Bool)shouldIgnoreEntranceSource:(id)arg1;
+ (_Bool)isAppLoginBindEnabled;
@property(retain, nonatomic) OPConfigCenter *configCenter; // @synthesize configCenter=_configCenter;
@property(retain, nonatomic) id <OPAdapterProtocol> adapterPlugin; // @synthesize adapterPlugin=_adapterPlugin;
