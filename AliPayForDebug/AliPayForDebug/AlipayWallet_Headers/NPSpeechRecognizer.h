//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NlsRecognizerDelegate-Protocol.h"

@class NPSoundEffect, NSString, NlsRecognizer;

@interface NPSpeechRecognizer : NSObject <NlsRecognizerDelegate>
{
    NPSoundEffect *asrStartSound;
    NPSoundEffect *asrGetSound;
    NPSoundEffect *asrErrorSound;
    _Bool _isStarted;
    NlsRecognizer *_recognizer;
    NSString *_bizId;
    NSString *_appId;
}

+ (_Bool)isServiceAvailable;
@property(copy, nonatomic) NSString *appId; // @synthesize appId=_appId;
@property(copy, nonatomic) NSString *bizId; // @synthesize bizId=_bizId;
@property(retain, nonatomic) NlsRecognizer *recognizer; // @synthesize recognizer=_recognizer;
@property(nonatomic) _Bool isStarted; // @synthesize isStarted=_isStarted;
- (void).cxx_destruct;
- (void)recognizer:(id)arg1 didCompleteRecognizingWithResult:(id)arg2 error:(id)arg3;
- (void)dealloc;
- (void)cancelRecognizer;
- (void)stopRecognizer;
- (void)asrStartRecognizer;
- (void)startRecognizerCompleteBlock:(CDUnknownBlockType)arg1;
- (void)initSoundFile;
- (id)initWithAppkey:(id)arg1 appid:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

