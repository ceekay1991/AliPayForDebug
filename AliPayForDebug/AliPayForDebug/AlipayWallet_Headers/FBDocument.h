//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "UIGestureRecognizerDelegate-Protocol.h"

@class FBFakeJSAPI, FBView, JSContext, NSMutableArray, NSMutableDictionary, NSMutableSet, NSString, UIScrollView, UITapGestureRecognizer, UITextField, UIView;
@protocol FBDocumentDelegate;

@interface FBDocument : UIViewController <UIGestureRecognizerDelegate>
{
    struct fb_core *_core;
    NSMutableDictionary *_viewDict;
    FBView *_root;
    UIScrollView *_scrollView;
    UITapGestureRecognizer *_tapGestureRecognizer;
    struct UIEdgeInsets _originalInsets;
    _Bool _navBar;
    _Bool _implicitScroll;
    id <FBDocumentDelegate> _delegate;
    id <FBDocumentDelegate> _fallbackDelegate;
    NSString *_docName;
    NSMutableDictionary *_dictLabel;
    NSMutableDictionary *_dictImg;
    NSMutableArray *_arrayScrollView;
    NSMutableDictionary *_timerMgr;
    NSMutableDictionary *_bizData;
    NSMutableDictionary *_bizParam;
    UITextField *_focusInput;
    UIView *_focusView;
    JSContext *_ctx;
    NSMutableSet *_asyncCallBack;
    FBFakeJSAPI *_fakeJSAPI;
    struct CGRect _frame;
}

+ (id)docWithPath:(id)arg1 data:(id)arg2 delegate:(id)arg3;
+ (id)doc:(id)arg1 data:(id)arg2;
@property(retain, nonatomic) FBFakeJSAPI *fakeJSAPI; // @synthesize fakeJSAPI=_fakeJSAPI;
@property(retain, nonatomic) NSMutableSet *asyncCallBack; // @synthesize asyncCallBack=_asyncCallBack;
@property(retain, nonatomic) JSContext *ctx; // @synthesize ctx=_ctx;
@property(nonatomic) __weak UIView *focusView; // @synthesize focusView=_focusView;
@property(nonatomic) __weak UITextField *focusInput; // @synthesize focusInput=_focusInput;
@property(retain, nonatomic) NSMutableDictionary *bizParam; // @synthesize bizParam=_bizParam;
@property(retain, nonatomic) NSMutableDictionary *bizData; // @synthesize bizData=_bizData;
@property(retain, nonatomic) NSMutableDictionary *timerMgr; // @synthesize timerMgr=_timerMgr;
@property(retain, nonatomic) NSMutableArray *arrayScrollView; // @synthesize arrayScrollView=_arrayScrollView;
@property(retain, nonatomic) NSMutableDictionary *dictImg; // @synthesize dictImg=_dictImg;
@property(retain, nonatomic) NSMutableDictionary *dictLabel; // @synthesize dictLabel=_dictLabel;
@property(nonatomic) _Bool implicitScroll; // @synthesize implicitScroll=_implicitScroll;
@property(readonly, nonatomic) _Bool navBar; // @synthesize navBar=_navBar;
@property(retain, nonatomic) NSString *docName; // @synthesize docName=_docName;
@property(nonatomic) __weak id <FBDocumentDelegate> fallbackDelegate; // @synthesize fallbackDelegate=_fallbackDelegate;
@property(nonatomic) __weak id <FBDocumentDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (void)handleTaps:(id)arg1;
- (void)keyboardWillHide:(id)arg1;
- (void)popupKeyboard:(id)arg1;
- (void)keyboardWillShow:(id)arg1;
- (void)setNeedLayout:(id)arg1;
- (void)layout:(id)arg1;
- (id)findViewBySelector:(id)arg1;
- (id)findViewByNode:(void *)arg1;
- (void)removeView:(struct fb_node *)arg1 fbView:(id)arg2;
- (id)getViewById:(id)arg1;
- (id)getView;
- (id)view;
@property(nonatomic) struct CGRect frame; // @synthesize frame=_frame;
- (void)reloadData:(id)arg1;
- (_Bool)hasOnReloadCb;
- (_Bool)checkFunc:(id)arg1;
- (void)updateLayout;
- (id)executeJSWithResult:(id)arg1;
- (void)executeJS:(id)arg1;
- (void)executeGlobalFunc:(id)arg1 andData:(id)arg2;
- (_Bool)focus;
- (long long)preferredInterfaceOrientationForPresentation;
- (unsigned long long)supportedInterfaceOrientations;
- (_Bool)shouldAutorotate;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (const char *)invokeNode:(void *)arg1 withAction:(const char *)arg2 andParam:(const char *)arg3;
- (void)dealloc;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 docName:(id)arg3;
- (id)initWithHtml:(id)arg1 withData:(id)arg2;
- (void)reLayout:(float)arg1 winHeight:(float)arg2;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 andFallbackDelegate:(id)arg4 andOption:(long long)arg5 docName:(id)arg6;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 andOption:(long long)arg4 docName:(id)arg5;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 andOption:(long long)arg4;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 docName:(id)arg4;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 andFallbackDelegate:(id)arg4 docName:(id)arg5;
- (id)initWithHtml:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3;
- (id)initWithFilePath:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3 docName:(id)arg4;
- (id)initWithFilePath:(id)arg1 withData:(id)arg2 andDelegate:(id)arg3;
- (void)logLoadView;
- (id)pageID;
- (void)setTemplateLogMark:(id)arg1;
- (id)templateLogMark;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

