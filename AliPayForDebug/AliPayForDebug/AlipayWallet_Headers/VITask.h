//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSDate, NSDictionary, NSMutableArray, NSString, UINavigationController, UIViewController, VITaskManager;
@protocol VIVersionStrategy;

@interface VITask : NSObject
{
    NSMutableArray *_modules;
    NSDate *_startDate;
    CDUnknownBlockType _onComplete;
    UIViewController *_taskFirstRootVC;
    _Bool _isNotFirstModule;
    unsigned long long _moduleCreateTime;
    NSMutableArray *_taskObservers;
    _Bool _didEnterBackground;
    _Bool _isExiting;
    _Bool _uninterrupt;
    _Bool _isSupportMultInstance;
    _Bool _isSupportMultInstanceMandatory;
    NSString *_token;
    NSString *_verifyId;
    NSString *_verifyType;
    VITaskManager *_taskManager;
    CDUnknownBlockType _eventProxy;
    UINavigationController *_currentNavigationController;
    NSString *_envType;
    NSString *_finishParams;
    NSDictionary *_extParams;
    NSString *_bizRequestData;
    id <VIVersionStrategy> _strategyContext;
    double _timeoutInterval;
    unsigned long long _initTaskRPCInterval;
    unsigned long long _taskCreateTime;
}

@property(nonatomic) _Bool isSupportMultInstanceMandatory; // @synthesize isSupportMultInstanceMandatory=_isSupportMultInstanceMandatory;
@property(nonatomic) _Bool isSupportMultInstance; // @synthesize isSupportMultInstance=_isSupportMultInstance;
@property(nonatomic) _Bool uninterrupt; // @synthesize uninterrupt=_uninterrupt;
@property(nonatomic) unsigned long long taskCreateTime; // @synthesize taskCreateTime=_taskCreateTime;
@property(nonatomic) unsigned long long initTaskRPCInterval; // @synthesize initTaskRPCInterval=_initTaskRPCInterval;
@property(nonatomic) double timeoutInterval; // @synthesize timeoutInterval=_timeoutInterval;
@property(retain, nonatomic) id <VIVersionStrategy> strategyContext; // @synthesize strategyContext=_strategyContext;
@property(retain, nonatomic) NSString *bizRequestData; // @synthesize bizRequestData=_bizRequestData;
@property(retain, nonatomic) NSDictionary *extParams; // @synthesize extParams=_extParams;
@property(retain, nonatomic) NSString *finishParams; // @synthesize finishParams=_finishParams;
@property(retain, nonatomic) NSString *envType; // @synthesize envType=_envType;
@property(retain, nonatomic) UINavigationController *currentNavigationController; // @synthesize currentNavigationController=_currentNavigationController;
@property(nonatomic) _Bool isExiting; // @synthesize isExiting=_isExiting;
@property(copy, nonatomic) CDUnknownBlockType eventProxy; // @synthesize eventProxy=_eventProxy;
@property(nonatomic) __weak VITaskManager *taskManager; // @synthesize taskManager=_taskManager;
@property(retain, nonatomic) NSString *verifyType; // @synthesize verifyType=_verifyType;
@property(retain, nonatomic) NSString *verifyId; // @synthesize verifyId=_verifyId;
@property(retain, nonatomic) NSString *token; // @synthesize token=_token;
- (void).cxx_destruct;
- (void)monitorQuitSDKWithCallbackParams:(id)arg1;
- (void)monitorQuitModuleWithResponse:(id)arg1;
- (void)monitorStartModuleWithResponse:(id)arg1;
- (void)monitorStartFirstModuleByResponse:(id)arg1;
- (void)dealloc;
- (void)disableInteractivePopGesture;
- (void)enableInteractivePopGesture;
- (void)invokeObserverForKey:(unsigned long long)arg1;
- (void)addTaskObserver:(id)arg1;
- (id)currentModule;
- (void)cancel;
- (void)invalidate;
- (void)installCertSlientlyWithParams:(id)arg1;
- (void)doCallbackWithParams:(id)arg1;
- (void)exitTaskWithParams:(id)arg1 onDealy:(double)arg2;
- (void)callbackWithParams:(id)arg1 animated:(_Bool)arg2;
- (_Bool)isLiving;
- (id)getCurrentSceneId;
- (void)finishWithParams:(id)arg1 animated:(_Bool)arg2;
- (void)finishWithResponse:(id)arg1 animated:(_Bool)arg2;
- (void)handleModule:(id)arg1 response:(id)arg2 animated:(_Bool)arg3;
- (void)handleEvent:(id)arg1;
- (void)verifyNextModule:(id)arg1;
- (void)updateModuleWithRequest:(id)arg1;
- (void)updateTaskContext;
- (void)startTask;
- (void)execute;
- (id)initWithKeyParams:(id)arg1 extParams:(id)arg2 onComplete:(CDUnknownBlockType)arg3;
- (id)initWithExtParams:(id)arg1 onComplete:(CDUnknownBlockType)arg2;
- (void)didEnterBackgroundHandler;
- (void)startInstallCertSilently;
- (id)initWithCompletion:(CDUnknownBlockType)arg1;

@end

