//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface RVAService : NSObject
{
    CDStruct_7976d84f _myFlags;
}

+ (id)allocPrivate;
+ (id)sharedService;
- (void)unregisterPluginCfgs:(id)arg1;
- (void)unregisterPluginCfg:(id)arg1;
- (void)registerPluginCfgs:(id)arg1;
- (void)registerPluginCfg:(id)arg1;
- (void)unregisterJSApiCfgs:(id)arg1;
- (void)unregisterJSApiCfg:(id)arg1;
- (void)registerJSApiCfgs:(id)arg1;
- (void)registerJSApiCfg:(id)arg1;
- (id)plugin:(id)arg1;
- (id)plugin:(id)arg1 groupId:(id)arg2;
- (id)plugins;
- (id)plugins:(id)arg1;
- (id)jsApis;
- (id)jsApis:(id)arg1;
- (id)jsApi:(id)arg1;
- (id)jsApi:(id)arg1 groupId:(id)arg2;
- (void)unRegisterJsApis:(id)arg1;
- (void)unregisterJSApi:(id)arg1;
- (void)registerJSApis:(id)arg1;
- (void)registerJSApi:(id)arg1;
- (void)registerJSApis:(id)arg1 groupId:(id)arg2;
- (void)registerJSApi:(id)arg1 groupId:(id)arg2;
- (id)createRVAViewController:(id)arg1 viewControllerClass:(Class)arg2 contentViewClass:(Class)arg3 withExpando:(id)arg4;
- (id)createRVAViewController:(id)arg1 withExpando:(id)arg2;
- (id)createRVAViewController:(id)arg1 viewControllerClass:(Class)arg2 contentViewClass:(Class)arg3;
- (id)createRVAViewController:(id)arg1;
- (_Bool)startSession:(id)arg1 animated:(_Bool)arg2;
- (void)start;
- (id)createRVAViewController:(id)arg1 currentPsdSession:(id)arg2 currentPsdScene:(id)arg3;
- (void)doStartHandler;
- (id)initPrivate;

@end

