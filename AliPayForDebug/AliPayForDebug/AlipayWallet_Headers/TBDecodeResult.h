//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSData, NSMutableDictionary, NSString;

@interface TBDecodeResult : NSObject
{
    int _subType;
    NSString *_resultType;
    NSString *_data;
    NSData *_rawData;
    NSString *_hiddenData;
    unsigned long long _stringEncode;
    NSString *_hmCodeTypeName;
    NSMutableDictionary *_extData;
    struct CGRect _bounds;
}

@property(retain, nonatomic) NSMutableDictionary *extData; // @synthesize extData=_extData;
@property(readonly, nonatomic) NSString *hmCodeTypeName; // @synthesize hmCodeTypeName=_hmCodeTypeName;
@property(nonatomic) unsigned long long stringEncode; // @synthesize stringEncode=_stringEncode;
@property(nonatomic) struct CGRect bounds; // @synthesize bounds=_bounds;
@property(retain, nonatomic) NSString *hiddenData; // @synthesize hiddenData=_hiddenData;
@property(retain, nonatomic) NSData *rawData; // @synthesize rawData=_rawData;
@property(retain, nonatomic) NSString *data; // @synthesize data=_data;
@property(nonatomic) int subType; // @synthesize subType=_subType;
@property(retain, nonatomic) NSString *resultType; // @synthesize resultType=_resultType;
- (void).cxx_destruct;
- (void)addExtDataWithKey:(id)arg1 value:(id)arg2;
- (id)initWithType:(int)arg1 subType:(int)arg2 data:(id)arg3 hiddenData:(id)arg4 bounds:(struct CGRect)arg5 StringEncode:(unsigned long long)arg6 RawData:(id)arg7 CodeTypeName:(id)arg8;

@end

